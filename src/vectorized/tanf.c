//! vectorized/tanf.c
//!
//! vectorized implementation of `tanf(x)`.
    ...
    for (size_t chunk = 0; chunk < vector_count; ++chunk) {
        ...
        if (vmaxvq_u32(special) != 0) {
            for (size_t lane = 0; lane < LANES; ++lane) {
                out.data[offset + lane] = vex_tanf(v.data[offset + lane]);
            }
        ...
        uint32x4_t use_cotangent = vtstq_u32(
            vreinterpretq_u32_s32(n),
            vdupq_n_u32(1)
        );

        float32x4_t numerator = vbslq_f32(
            use_cotangent,
            cos_r,
            sin_r
        );
        float32x4_t denominator = vbslq_f32(
            use_cotangent,
            sin_r,
            cos_r
        );
        ...
        uint32x4_t result_sign = veorq_u32(
            input_sign,
            vandq_u32(use_cotangent, sign_bit)
        );
