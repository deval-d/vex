...
int scalar(void) {
    ...
    printf("scalar vex expf:  %.3f ns\n", vex_time * 1e9); 
    printf("scalar math expf: %.3f ns\n", math_time * 1e9); 
...
int vectorized(void) {
    ...
    double acc_time = benchmark_vector_float(
        ...
        output,
        n,
        benchmark_accelerate_vexpf,
        iterations
    );

    printf("vector length:     %zu\n", n);
    printf("vex vexpf:         %.3f us, %.3f ns/element\n", vex_time * 1e6, vex_time * 1e9 / n);
    printf("accelerate vvexpf: %.3f us, %.3f ns/element\n", acc_time * 1e6, acc_time * 1e9 / n); 
